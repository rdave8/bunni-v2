...
import "../src/ldf/managed/CuratedDistribution.sol";
...
contract BunniHookTest is BaseTest {
    ...
    function test_curatedDistributionLdfPoC() external {
        ...
        bytes28 newBaseParams = bytes28(
            abi.encodePacked(
                ShiftMode.BOTH, int24(-10), uint16(5), uint32(1.1e8), uint32(1e6), bytes14("12345678912345")
            )
        );
        ...
        // fulfill order
        _mint(Currency.wrap(order.offer[0].token), address(this), order.consideration.amount);
        (uint256 beforeBalance0, uint256 beforeBalance1) = hub.poolBalances(key.toId());
        uint256 beforeFulfillerBalance = currencyOut.balanceOfSelf();
